...
In [Your First R Project](../part1/first-project.qmd), you used renv to manage R packages for your project. Conda serves the same purpose for Python: isolated, reproducible environments with pinned package versions. The concepts are the same—the tooling is different.
...
Each project has its own libraries specific to its needs. Updates to libraries or the addition of new libraries in one project are isolated from other projects.
...
You only need to create the environment once. New environments can be initiated with just a python installation, or include additional packages from the start. The option `-n project_name` specifies the name of the environment, and is followed by an initial list of packages to install:
...
When setting up a new project environment, common data science packages you might include are:
...
| `ipykernel` | **Required** for Quarto to execute Python `.qmd` files |
| `seaborn` | Statistical data visualization (built on matplotlib) |
| `session-info` | Python session info for reproducibility (install via pip) |

A typical environment creation command:

```bash
conda create -n my-project python=3.11 numpy pandas matplotlib seaborn ipykernel -y
...
Every data science environment must include `ipykernel`. Without it, Quarto cannot execute Python `.qmd` files — you'll get a kernel error when rendering. Make it a habit to include it in every `conda create` command.
:::
...
> I'm getting `ModuleNotFoundError: No module named 'pandas'` when I run my script, but I thought I installed it. The environment is `my-project` and here's what `conda list` shows: [paste output]. What's wrong?

Claude will check whether the package is installed in the right environment, whether your script is running in a different environment, and suggest the fix.
:::
