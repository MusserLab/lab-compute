...
You only need to do this once per project. If you use the Musser Lab's `/new-project` skill in Claude Code, it sets the R version automatically when scaffolding a new project. See the [Positron chapter](positron.qmd#setting-the-r-version) for more details.
...
The default renv setup handles the `.gitignore` entries automatically. If you used the lab's project setup walkthrough or the `/new-project` skill, these exclusions are already in place.
