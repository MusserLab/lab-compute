...
The YAML header controls document metadata and rendering behavior. Here's a typical header for a lab analysis script:
...
| `cache` | Cache chunk results | `false` (usually) |

Setting `message: false` and `warning: false` globally keeps your rendered output clean. You can override these per-chunk when you need to see specific messages.

## The Setup Chunk

Every analysis script should start with a setup chunk that loads packages, defines paths, and sets options. This establishes the foundation for everything that follows.
...
dir_out <- here::here("outs", "01_my_analysis")
...
After the YAML header and before the setup chunk, document what the script reads and produces:

```markdown
# Introduction

This script generates volcano plots for the phosphoproteomics time course,
visualizing differential phosphorylation between treated and control samples.
...
- `outs/01_volcano_plots/volcano_*.pdf` — volcano plots per comparison
- `outs/01_volcano_plots/significant_hits.csv` — filtered results table
```

This practice:

- Makes the script's purpose immediately clear
- Documents dependencies (what must exist before this runs)
- Documents products (what other scripts can use as inputs)
- Helps you and collaborators understand the analysis pipeline
...
End your script with session information for reproducibility:
...
This records R version, package versions, and platform information in the rendered output.
...
>
> The code runs fine when I execute it interactively in the console. What's going wrong?

*Include the exact error. Mention if it works interactively—that's an important clue that often points to missing objects or path issues.*
:::
...
dir_out <- here::here("outs", "script_name")
dir.create(dir_out, recursive = TRUE, showWarnings = FALSE)
