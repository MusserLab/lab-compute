...
Python QMD files require the conda environment to be active when you render:
...
This section collects the Python-specific details that don't fit neatly into the tabsets above.
...
**Install `session-info`.** For the provenance chunk to work, install `session-info` in your conda environment: `pip install session-info`.
