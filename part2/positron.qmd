...
In [Your First R Project](../part1/first-project.qmd), you used Positron to run a single-cell analysis, write code in the editor, execute it in the console, inspect objects in the Variables pane, and view plots in the plotting window. This chapter digs deeper into how Positron is set up and how to use it effectively in your projects. We’ll cover what the panels actually are, how to manage multiple sessions, and a few habits that make daily work smoother.

It was only a couple months ago that I personally switched from RStudio to Positron, and it has quickly become my favorite IDE (Integrated Development Environment). It’s where I write code, explore data, run analyses, and manage ongoing projects. Built by Posit, the company behind RStudio, Positron keeps the best RStudio-style data science features (like the plotting pane and environment viewer) while borrowing the best parts of VS Code, especially file navigation, Git and GitHub workflows, and extensions like Claude Code that integrate directly into your projects.

## Why Positron?

If you’re coming from RStudio like me, Positron will feel familiar. You still get an R console, a variable explorer, a data viewer, and a file browser. If you’ve used VS Code before, it’ll also feel familiar, just with a lot more data-science visualization and environment management tools. A few features that I find most useful are:

**Full support for both R and Python.** More and more of our projects rely on both languages. A lot of classic workflows are still anchored in R (for example, Seurat), but essential tools increasingly live in Python (e.g., Scanpy). In Positron, you can run R and Python sessions side by side, each with its own console and environment viewer. In practice I usually write one language per script (`.qmd` file), but I often use both within a project, sometimes even at the same time. It’s easy to switch between sessions, and the console, environment viewer, and plotting pane update to match whatever you’re working in.

**A superior Data Viewer.** One of my favorite things about Positron is the interactive Data Viewer, which works with both R and Python objects (for example, tibbles and pandas DataFrames), as well as TSV and CSV files from the file explorer. Click a data object in the Variables pane and it opens in a fast viewer where you can sort, filter, and search. For larger datasets it stays responsive and gives quick per-column summaries (including missingness). The Data Viewer is central to exploratory analysis, and you should get in the habit of using it constantly.

**Access to VS Code extensions.** Positron inherits access to thousands of extensions, most notably Claude Code and Codex, the two main coding assistants from Anthropic and OpenAI. This is a major difference from RStudio, which has relatively few extensions and cannot really support agentic AI workflows.
...
On Windows, Positron’s integrated terminal is PowerShell, not bash/zsh. Basic navigation commands work the same (cd, ls, mkdir), but some Unix-style options and utilities used in this guide may not. If a command fails, look for a Windows alternative in the text, or run the command in a bash-like shell (Git Bash or WSL) instead.
