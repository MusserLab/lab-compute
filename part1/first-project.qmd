...
Now let's inspect the cell metadata. Run the `peek-metadata` chunk, or type this in the console:

```r
View(sponge@meta.data)
```

This opens Positron's **data viewer**—a spreadsheet-style view of the metadata, with one row per cell. You'll see three columns:
...
DotPlot(sponge, features = top_genes) + RotatedAxis()
```
